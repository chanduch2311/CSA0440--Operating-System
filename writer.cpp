...
int main() {
    ...
    strncpy(shm_ptr, buffer, SHM_SIZE - 1);  // leave room for null terminator
    ...
    // Wait for the reader to finish reading by checking for '%'
    while (*shm_ptr != '%') {
        sleep(1);
    }
    ...
    printf("Shared memory has been cleaned up.\n");
    return 0;
}
